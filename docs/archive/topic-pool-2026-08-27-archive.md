...
| **K1**：D24-B 字数控制软化（**2026-09-01**）| D16 5-type 硬上限 → soft 建议（advisory 不阻 commit）+ 全档通用 ceiling ≤ 3500 词；反 Scaled Content 主防线明确为结构多样性 + 指纹段 + 真实 log | `docs/writing-prompts.md` §一/§二-§六/§七/§八/§九 + `docs/article-writing-workflow.md` §5.2.1 + `README.md` §6 D16 段 + `topic-pool-archive.md` 顶部 D16 注释 + 5 篇 published 文章 front matter 注释 + 4 选题预计字数标注 | ✅ **已完成**（本会话）|
| **L1**：D24 Hub prompt_type 决策（**2026-09-01**）| 5-type taxonomy 不覆盖聚簇索引结构 → 新增 **F · 聚簇索引型**（Hub-and-Spoke V1 拓扑的 Hub / 类别落地页 / FAQ index）；Hub 归属 F，soft ≤2500 + 3500 ceiling 合规 | `docs/writing-prompts.md` §一表 + 新增§七 Prompt F + §八 共性原则「6 种 prompt 通用」+ §九 参考 + Hub front matter `prompt_type = "F"` + `docs/article-writing-workflow.md` §5.2.1 8 文章表 + `scripts/lint-post.sh` 扩 `[A-E]` → `[A-F]` | ✅ **已完成**（本会话）|
